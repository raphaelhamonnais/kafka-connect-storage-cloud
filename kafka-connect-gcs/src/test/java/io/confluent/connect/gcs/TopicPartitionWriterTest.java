...
import io.confluent.connect.gcs.util.TimeUtils;
...
public class TopicPartitionWriterTest extends TestWithMockedGcs {
  // The default
  ...
  public void setUp() throws Exception {
    ...
    Format<GcsSinkConnectorConfig, String> format = new AvroFormat(storage);
  ...
  public void testWriteRecordDefaultWithPaddingSeq() throws Exception {
    localProps.put(GcsSinkConnectorConfig.FILENAME_OFFSET_ZERO_PAD_WIDTH_CONFIG, "2");
    setUp();
  ...
  public void testWriteRecordFieldPartitioner() throws Exception {
    localProps.put(GcsSinkConnectorConfig.FLUSH_SIZE_CONFIG, "9");
    setUp();
  ...
  public void testWriteRecordTimeBasedPartitionRecordTimestampHours() throws Exception {
    ...
    parsedConfig.put(GcsSinkConnectorConfig.ROTATE_INTERVAL_MS_CONFIG, TimeUnit.MINUTES.toMillis(1));
  ...
  public void testWallclockUsesBatchTimePartitionBoundary() throws Exception {
    localProps.put(GcsSinkConnectorConfig.FLUSH_SIZE_CONFIG, "6");
    setUp();
  ...
  public void testNoFilesWrittenWithoutCommit() throws Exception {
    ...
    localProps.put(GcsSinkConnectorConfig.FLUSH_SIZE_CONFIG, "10");
    setUp();
  ...
  private void verify(List<String> expectedFileKeys, int expectedSize, Schema schema, List<Struct> records)
      throws IOException {
    List<Blob> summaries = listObjects(GCS_TEST_BUCKET_NAME, null, gcs);
    List<String> actualFiles = new ArrayList<>();
    for (Blob blob : summaries) {
    ...
    for (String fileKey : actualFiles) {
      Collection<Object> actualRecords = readRecordsAvro(GCS_TEST_BUCKET_NAME, fileKey, gcs);
