...
import io.confluent.connect.gcs.format.bytearray.ByteArrayFormat;
...
//    assertNotNull(gcs.get(GCS_TEST_BUCKET_NAME));
  ...
  public void testBufferOverflowFix() throws Exception {
    localProps.put(GcsSinkConnectorConfig.FORMAT_CLASS_CONFIG, ByteArrayFormat.class.getName());
    setUp();
    PowerMockito.doReturn(5).when(connectorConfig).getPartSize();
    GcsOutputStream out = new GcsOutputStream(GCS_TEST_BUCKET_NAME, connectorConfig, gcs);
  ...
  public void testGzipCompression() throws Exception {
    ...
    localProps.put(GcsSinkConnectorConfig.FORMAT_CLASS_CONFIG, ByteArrayFormat.class.getName());
    localProps.put(GcsSinkConnectorConfig.COMPRESSION_TYPE_CONFIG, compressionType.name);
    setUp();
    task = new GcsSinkTask(connectorConfig, context, storage, partitioner, format, SYSTEM_TIME);
  ...
  public void testCustomExtensionAndLineSeparator() throws Exception {
    ...
    localProps.put(GcsSinkConnectorConfig.FORMAT_CLASS_CONFIG, ByteArrayFormat.class.getName());
    localProps.put(GcsSinkConnectorConfig.FORMAT_BYTEARRAY_LINE_SEPARATOR_CONFIG, "SEPARATOR");
    localProps.put(GcsSinkConnectorConfig.FORMAT_BYTEARRAY_EXTENSION_CONFIG, extension);
    setUp();
    task = new GcsSinkTask(connectorConfig, context, storage, partitioner, format, SYSTEM_TIME);
