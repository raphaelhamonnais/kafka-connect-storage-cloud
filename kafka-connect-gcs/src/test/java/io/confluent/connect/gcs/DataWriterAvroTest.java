...
public class DataWriterAvroTest extends TestWithMockedGcs {
  ...
  public void setUp() throws Exception {
    ...
    gcs = newGcsClient(connectorConfig);
    storage = new GcsStorage(connectorConfig, url, GCS_TEST_BUCKET_NAME, gcs);
  ...
  public void testWriteRecords() throws Exception {
    setUp();
    task = new GcsSinkTask(connectorConfig, context, storage, partitioner, format, SYSTEM_TIME);
  ...
  public void testCompressFile() throws Exception {
    ...
    List<Blob> blobs = listObjects(GCS_TEST_BUCKET_NAME, "/", gcs);
    for(Blob blob: blobs) {
      InputStream in = inputStream(gcs, blob.getBucket(), blob.getName());
  ...
  public void testPreCommitOnSizeRotation() throws Exception {
    localProps.put(GcsSinkConnectorConfig.FLUSH_SIZE_CONFIG, "3");
    setUp();
    task = new GcsSinkTask(connectorConfig, context, storage, partitioner, format, SYSTEM_TIME);
    ...
    List<SinkRecord> sinkRecordgcs = createRecordsInterleaved(context.assignment().size(), 5, context.assignment());

    task.put(sinkRecordgcs);
    offsetsToCommit = task.preCommit(null);

    long[] validOffsetgcs = {6, 6};
    verifyOffsets(offsetsToCommit, validOffsetgcs, context.assignment());
  ...
  public void testPreCommitOnSchemaIncompatibilityRotation() throws Exception {
    localProps.put(GcsSinkConnectorConfig.FLUSH_SIZE_CONFIG, "2");
    setUp();
  ...
  public void testPreCommitOnRotateTime() throws Exception {
    ...
    localProps.put(GcsSinkConnectorConfig.FLUSH_SIZE_CONFIG, "1000");
    localProps.put(
        GcsSinkConnectorConfig.ROTATE_INTERVAL_MS_CONFIG,
    ...
    task = new GcsSinkTask(connectorConfig, context, storage, partitioner, format, time);
  ...
  public void testPreCommitOnRotateScheduleTime() throws Exception {
    ...
    localProps.put(GcsSinkConnectorConfig.FLUSH_SIZE_CONFIG, "1000");
    localProps.put(
        GcsSinkConnectorConfig.ROTATE_SCHEDULE_INTERVAL_MS_CONFIG,
        String.valueOf(TimeUnit.HOURS.toMillis(1))
    );
    setUp();
  ...
  public void testRebalance() throws Exception {
    ...
    long[] validOffsetgcs = {6, 9, 12};
    verify(sinkRecords, validOffsetgcs, Collections.singleton(TOPIC_PARTITION3), true);
    ...
    expectedFiles.addAll(getExpectedFiles(validOffsetgcs, TOPIC_PARTITION3));
  ...
  protected void verifyFileListing(List<String> expectedFiles) throws IOException {
    List<Blob> blobs = listObjects(GCS_TEST_BUCKET_NAME, null, gcs);
    List<String> actualFiles = new ArrayList<>();
    for (Blob blob : blobs) {
      String fileKey = blob.getName();
   ...
   * @param sinkRecords a flat list of the records that need to appear in potentially several files in Gcs.
    ...
    for (TopicPartition tp : partitions) {
      for (int i = 1, j = 0; i < validOffsets.length; ++i) {
        ...
        Collection<Object> records = readRecords(topicsDir, getDirectory(tp.topic(), tp.partition()), tp, startOffset,
                                                 extension, ZERO_PAD_FMT, GCS_TEST_BUCKET_NAME, gcs);
